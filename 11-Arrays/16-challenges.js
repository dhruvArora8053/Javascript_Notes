...
const checkDogs = function (dogsJulia, dogsKate) {
  const shallowDogsJulia = dogsJulia.slice().splice(1, 2);
  console.log(shallowDogsJulia);

  const kateJuliaDogs = [...shallowDogsJulia, ...dogsKate];
  console.log(kateJuliaDogs);

  kateJuliaDogs.forEach((dog, i) => {
    dog >= 3
...
const calcAverageHumanAge = function (allDogs) {
  const dogHumanAge = allDogs
    .map((dog) => (dog <= 2 ? 2 * dog : 16 + dog * 4))
    .filter((dog) => dog >= 18);
  console.log(dogHumanAge);

  const avg = dogHumanAge.reduce((acc, dog) => acc + dog) / dogHumanAge.length;
  console.log(avg);
};
...
const calcAverageHumanAge1 = function (allDogs) {
  const dogHumanAge = allDogs
    .map((dog) => (dog <= 2 ? 2 * dog : 16 + dog * 4))
    .filter((dog) => dog >= 18)
